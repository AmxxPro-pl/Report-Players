...
enum _:Cvars { prefix[64], interval, report_flag[8], report_admin, show_admin, show_time, show_map, show_reason, 
               my_reason, show_player, show_ping, id_ranga[64], bot_name[64], bot_avatar[64], title[64], show_footer_img,
               footer_img[64], color[32], thumbnail_img[64], show_thumbnail_img }
new gTarget[33], gReason[33][192], rMenu, mapname[32], buffer[LENGTH][128], gTime[9], amxxpro_discord[Cvars];
...
public plugin_init()
{
    ...
    //Avatar BOT'a
    bind_pcvar_string(create_cvar("amxxpro_discord_bot_avatar", "https://i.imgur.com/EDUv58r.png"), amxxpro_discord[bot_avatar], charsmax(amxxpro_discord[bot_avatar]))
    ...
    bind_pcvar_string(create_cvar("amxxpro_discord_footer_img", "https://i.imgur.com/nlCnT4I.png"), amxxpro_discord[footer_img], charsmax(amxxpro_discord[footer_img]));  

    //Kolor EMBED ( 12092939 - domyslnie)
    bind_pcvar_string(create_cvar("amxxpro_discord_color", "12092939"), amxxpro_discord[color], charsmax(amxxpro_discord[color]))

    //Grafika thumbnail
    bind_pcvar_string(create_cvar("amxxpro_discord_thumbnail_img", "https://i.imgur.com/EDUv58r.png"), amxxpro_discord[thumbnail_img], charsmax(amxxpro_discord[thumbnail_img]));  
...
public create_reasons(){
        ...
        for(new i = 0; i < ArraySize(Powod); i++){
            ArrayGetString(Powod, i, powodzik, charsmax(powodzik));
            menu_additem(rMenu, fmt("\w[\r>\w] %s", powodzik));
        } 
...
public client_putinserver(id){
    ...
    if(!TrieGetCell(AntiF, buffer[STEAMID], AntiFlood[id]))
        AntiFlood[id] = 0;
}
public clear(id){
    gTarget[id] = 0;
...
public send_report(id)
{
    ...
    Discord_SetStringParam(AUTHOR_AVATAR, "https://i.imgur.com/EDUv58r.png");
